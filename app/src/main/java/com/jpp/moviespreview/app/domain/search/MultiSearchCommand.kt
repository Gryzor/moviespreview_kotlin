...
/**
 * [Command] executed to perform a multi-searchFirstPage (searchFirstPage any topic related to Movies).
 * It will execute the searchFirstPage with the provided [MultiSearchParam] and will set the
...
class MultiSearchCommand(private val mapper: MultiSearchDataMapper,
    ...
    override fun execute(data: CommandData<MultiSearchPage>, param: MultiSearchParam?) {
        ...
        api.multiSearch(param.query, page)?.let {
            ...
            data.error = IllegalStateException("Can execute searchFirstPage at this moment")
        }
    }
